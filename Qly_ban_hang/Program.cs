...
class Program
{
    ...
    static void SaleManagementMenu()
    {
        List<Sale> saleList = new List<Sale>();
        while (true)
        {
            DrawMenuFrame("Sale Management", new string[] {
            "1. View Sale List",
            "2. Add Sale",
            "3. Edit Sale",
            "4. Delete Sale",
            ...
            {
                case 1:
                    DisplaySale(saleList);
                    break;
                case 2:
                    AddSale(saleList);
                    break;
                case 3:
                    EditSaleInfo(saleList, connectionString);
                    break;
                case 4:
                    DeleteSaleInfo(saleList, connectionString);
                    break;
                case 5:
                    SearchSaleByID(saleList, connectionString);
                    break;
                case 0:
                    return;
                default:
    ...
    static void DisplaySale(List<Sale> saleList)
    {
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                while (reader.Read())
                {
                    Console.WriteLine($"Sale ID: {reader["id_sale"]}, Name: {reader["name"]}, Start Time: {reader["time_available"]}, End Time: {reader["time_end"]}");
                }
                reader.Close();
            }
            catch (Exception ex)
    ...
    static void AddSale(List<Sale> saleList)
    {
        Sale sale = new Sale();

        try
        {
            Console.WriteLine("Enter new sale information: ");
            ...
            sale.TimeAvailable = DateTime.Parse(Console.ReadLine());
            Console.Write("Enter end time (yyyy-MM-dd HH:mm:ss): ");
            sale.TimeEnd = DateTime.Parse(Console.ReadLine());

            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                string query = "INSERT INTO sale (id_sale, name, time_available, time_end) VALUES (@SaleID, @Name, @TimeAvailable, @TimeEnd)";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@SaleID", sale.SaleID);
                command.Parameters.AddWithValue("@Name", sale.Name);
                command.Parameters.AddWithValue("@TimeAvailable", sale.TimeAvailable);
                command.Parameters.AddWithValue("@TimeEnd", sale.TimeEnd);
                command.ExecuteNonQuery();
            }
    ...
    static void EditSaleInfo(List<Sale> saleList, string connectionString)
    {
        Console.Write("Enter sale ID to edit: ");
        int saleID = int.Parse(Console.ReadLine());
        Sale sale = saleList.Find(s => s.SaleID == saleID);
        ...
        if (sale != null)
        {
            try
            {
                ...
                {
                    connection.Open();
                    string query = "UPDATE sale SET name = @Name, time_available = @TimeAvailable, time_end = @TimeEnd WHERE id_sale = @SaleID";
                    MySqlCommand command = new MySqlCommand(query, connection);
                    command.Parameters.AddWithValue("@SaleID", sale.SaleID);
                    command.Parameters.AddWithValue("@Name", sale.Name);
                    command.Parameters.AddWithValue("@TimeAvailable", sale.TimeAvailable);
                ...
                Console.WriteLine("Sale information updated successfully!");
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error: " + ex.Message);
    ...
    static void DeleteSaleInfo(List<Sale> saleList, string connectionString)
    {
        ...
        try
        {
            ...
            {
                connection.Open();
                string query = "DELETE FROM sale WHERE id_sale = @SaleID";
                MySqlCommand command = new MySqlCommand(query, connection);
                ...
                if (rowsAffected > 0)
                {
                    saleList.RemoveAll(s => s.SaleID == saleID);
    ...
    static void SearchSaleByID(List<Sale> saleList, string connectionString)
    {
        Console.Write("Enter sale ID to find: ");
        int saleID = int.Parse(Console.ReadLine());
        ...
        try
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                string query = "SELECT * FROM sale WHERE id_sale = @SaleID";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@SaleID", saleID);
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        Console.WriteLine($"Sale ID: {reader["id_sale"]}");
                        Console.WriteLine($"Name: {reader["name"]}");
                        Console.WriteLine($"Start Time: {reader["time_available"]}");
                        Console.WriteLine($"End Time: {reader["time_end"]}");
                    }
                    else
                    {
                        Console.WriteLine("Sale with this ID not found.");
    ...
    static void DisplayOrder(List<Order> orderList)
    {
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            while (reader.Read())
            {
                Console.WriteLine($"Order ID: {reader["OrderID"]},Customer ID: {reader["CustomerID"]},Product ID: {reader["ProductID"]},Sale ID: {reader["SaleID"]},Order Quantity: {reader["OrderQuantity"]}, Order Date: {reader["OrderDate"]}, Shipping Address: {reader["ShippingAddress"]}, Total Amount: {reader["TotalAmount"]}");
            }
            reader.Close();
        }
    }
