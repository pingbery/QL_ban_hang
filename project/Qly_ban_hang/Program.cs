...
class Program
{
    ...
    static void Register()
    {
        ...
        while(invalidAttempts < maxAttempts)
        {
            ...
            if (IsValidUsername(username))
            {
                ...
                {
                    Console.WriteLine("Username already exists on the system. Please re-enter.");
                    invalidAttempts++;
                }
                else
                {
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            if (rowsAffected > 0)
            {
                Console.WriteLine("Regester Successfully.");
            }
        }
    }
    ...
    static bool IsUsernameExists(string username) // kiểm tra sự tồn tại username trong csdl
    {
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            connection.Open();
    ...
    static bool IsValidUsername(string username) // kiểm tra định dạng username
    {
        ...
        {
            Console.Write("Tên đăng nhập không được để trống.");
            return false;
        }
        else if (username.Length < 7 || username.Length > 16)
        {
            Console.Write("Tên đăng nhập phải có độ dài từ 7 đến 16 kí tự");
            return false;
        }
        else if (!regex.IsMatch(username))
        {
            Console.Write("Tên đăng nhập phải chứa ít nhất một chữ cái và một số, không chứa ksi tự đặc biệt và khoảng trắng");
            return false;
        }

        return true;
    }
    static bool IsValidPassword(string password) // kiểm tra định dạng mật khẩu
    {
        ...
        {
            Console.Write("Mật khẩu phải có độ dài từ 6 đến 12 kí tự");
            return false;
        }
        else if (!regex.IsMatch(password))
        {
            Console.Write("Mật khẩu phải chứa ít nhất một chữ cái, một số, một kí tự đặc biệt và không được để trống");
            return false;
        }
        else
        {
    ...
    static bool IsEmailExists(string email) // kiểm tra sự tồn tại của email trong csdl
    {
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            connection.Open();
    ...
    static bool IsValidEmail(string email) // kiểm tra định dạng email
    {
        ...
        {
            Console.WriteLine("Email phải có định dạng example@example.com.");
            return false;
        }

        return true;
    }
    static bool IsPhoneNumberExists(string phoneNumber) // kiểm tra sự tồn tại của phone number trong csdl
    {
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            connection.Open();
    ...
    static bool IsValidPhoneNumber(string phoneNumber) // kiểm tra định dạng phone number
    {
        string pattern = @"^\d{10}$";
        Regex regex = new Regex(pattern);
        ...
        {
            Console.WriteLine("Số điện thoại phải là 10 chữ số");
            return false;
        }
...
{
    Console.Write("Nhập ID khách hàng cần chỉnh sửa: ");
    ...
    using (MySqlConnection connection = new MySqlConnection(connectionString))
    {
        ...
        using (MySqlDataReader reader = command.ExecuteReader())
        {
            if (reader.Read())
            {
                ...
                string updateQuery = "UPDATE customer SET Name = @Name, Sex = @Gender, Email = @Email, PhoneNumber = @Phone, Address = @Address, DateOfBirth = @DateOfBirth WHERE CustomerID = @CustomerID";
                ...
                updateCommand.Parameters.AddWithValue("@Gender", customer.Sex);
                updateCommand.Parameters.AddWithValue("@Email", customer.Email);
                updateCommand.Parameters.AddWithValue("@Phone", customer.Phonenumber);
                ...
                int index = customerList.FindIndex(c => c.CustomerID == customerID);
                if (index != -1)
                {
    ...
    static void DisplayOrder(List<Order> orderList)
    {
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            while (reader.Read())
            {
                Console.WriteLine($"Order ID: {reader["OrderID"]},Customer ID: {reader["CustomerID"]},Product ID: {reader["ProductID"]},Coupon Code: {reader["CouponCode"]},Order Quantity: {reader["OrderQuantity"]}, Order Date: {reader["OrderDate"]}, Shipping Address: {reader["ShippingAddress"]}, Total Amount: {reader["TotalAmount"]}");
            }
            reader.Close();
        }
    }
    static void AddOrder(List<Order> orderList, string connectionString)
    {
        ...
        {
            connection.Open();
            string query = "INSERT INTO order (OrderID, CustomerID, ProductID, CouponCode,OrderQuantity, OrderDate, ShippingAddress, TotalAmount) VALUES (@OrderID, @CustomerID, @ProductID, @CouponCode,@OrderQuantity, @OrderDate, @ShippingAddress, @TotalAmount)";
            MySqlCommand command = new MySqlCommand(query, connection);
    ...
    static void EditOrderInfo(List<Order> orderList, string connectionString)
    {
        ...
        if (order != null)
        {
            ...
            {
                connection.Open();
                string query = "UPDATE order SET CustomerID = @CustomerID, ProductID = @ProductID, Coupon Code = @CouponCode,OrderQuantity = @OrderQuantity, OrderDate = @OrderDate, ShippingAddress = @ShippingAddress, TotalAmount = @TotalAmount WHERE OrderID = @OrderID";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@OrderID", order.OrderID);
                command.Parameters.AddWithValue("@CustomerID", order.CustomerID);
                command.Parameters.AddWithValue("@ProductID", order.ProductID);
        ...
        using (MySqlConnection connection = new MySqlConnection(connectionString))
        {
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                if (reader.Read())
                {
                    ...
                    Console.WriteLine($"Customer ID: {reader["CustomerID"]}");
                    Console.WriteLine($"Product ID: {reader["ProductID"]}");
                    Console.WriteLine($"Coupon Code: {reader["Coupon Code"]}");
